...
#nullable enable
    public sealed class JTokenResourceWatcher : IDisposable

    {
        private const double MaxRetrySeconds = 32;

        private readonly Subject<(WatchEventType Event, JToken Resource)> _watchEvents = new();
        ...
        public IObservable<(WatchEventType Event, JToken Resource)> WatchEvents => _watchEvents;
        ...
        private void OnWatcherEvent(WatchEventType type, object resource)
        {
            ...
            switch (type)
            {
                ...
                case WatchEventType.Deleted:
                    if (resource is JsonElement content)
                    {
                        _watchEvents.OnNext((type, JToken.Parse(content.GetRawText())));
                    }
                    else
                    {
                        _logger.LogWarning(@"JTokenResourceWatcher: Received watch event {eventType} for {plural} but resource is not a JsonElement.", type, _plural);
                    }
