...
namespace Sentinel.Core.K8s.Watchers
{
    ...
    public class K8SEventOps
    {
        ...
        {
            var events = await _client.ApiClient.CoreV1.ListNamespacedEventWithHttpMessagesAsync(@namespace);
            return events.Body.Items;
        }

        public async Task<IList<Corev1Event>> GetAllAsync()
        {
            var events = await _client.ApiClient.CoreV1.ListEventForAllNamespacesWithHttpMessagesAsync();
            return events.Body.Items;
        }
            ...
            )
        {
            ...
            var savedevent = await this._client.ApiClient.CoreV1.CreateNamespacedEventAsync(newEvent, @namespace);
